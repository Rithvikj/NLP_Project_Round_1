...
# In this the book used is 'COMPUTER GRAPHICS C VERSION' by DONALD HEARN and M. PAULINE BAKER
nltk.download('punkt')
nltk.download('averaged_perceptron_tagger')
nltk.download('stopwords')

file = open('book2.txt', 'r', encoding='utf-8')
text = file.read()

file.close()
text = re.sub(r'[^\x00-\x7f]', " ", text)# Removing non-ASCII characters
text = re.sub( "\n[Cc]hapter \d+\n[^\n]+\n" , " ", text)# Removing 'Chapter 1' like words
text = re.sub( "\n[0-9]+-[0-9]+\n[^\n]+\n" , " ", text)# Removing '1-1' like text
text = re.sub( "\n[0-9]+\n" , " ", text)# Removing page numbers
text = re.sub( "[Ff]igure[ ][0-9]+-[0-9]+" , " ", text)# Removing 'Figure' keyword
